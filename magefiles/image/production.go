...
// This image used to be the production environment
// We now know better and only use it to produce an Elixir release:
// - https://github.com/thechangelog/changelog.com/discussions/402
...
// This is the release image that we run in production
// - https://hexdocs.pm/phoenix/releases.html#containers
func (image *Image) Release() *Image {
	release := image.Production().
		WithAppRelease().
		OK().
		container.
		Directory("/app.release")

	release.Export(image.ctx, "tmp/app.release")
	...
	image.container = image.container.
		WithDirectory("/app", release).
		WithWorkdir("/app").
		WithExec([]string{
			"echo", "Check release version...",
		}).
		WithExec([]string{
			"/app/bin/changelog", "version",
		})

	// 👉 TODO: Check GIT_AUTHOR, GIT_SHA & BUILD_URL
	image.container.Export(image.ctx, "tmp/release.tar")
...
func UbuntuImageRef() string {
	return fmt.Sprintf("ubuntu:%s", UbuntuVersionShort)
}
